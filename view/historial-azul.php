...
<body>
    ...
    <?php
        ...
        echo "<a href='../view/reservas-azul.php' class='btn btn-dark' style='padding-left: 60px; padding-right: 60px; border-color: white; background-color: #1c497a;'>Back</a>";
        echo "</div>";
        echo "<h1>Historial de reservas de la Sala Azul</h1>";
    ...
    <table class="tbl_eventos">
        ...
        if(!isset($_POST['enviar'])){
            $stmt = $pdo->prepare('SELECT * FROM tbl_reserva where id_mesa >= 17 and id_mesa <= 24 ORDER BY id_mesa ASC');
            $stmt->execute();
            ...
            $stmt = $pdo->prepare("SELECT * FROM tbl_reserva where id_mesa >= 17 and id_mesa <= 24 and id_mesa like '%$table%' and fecha_inicio like '%$dia%' and hora_reserva like '%$hora%' ORDER BY id_mesa ASC");
            $stmt->execute();
            $sentencia=$stmt->fetchAll(PDO::FETCH_ASSOC);
