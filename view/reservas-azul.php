...
<body>
    ...
    <table class="tbl_eventos">
        ...
        if(!isset($_POST['enviar'])){
            $stmt = $pdo->prepare('SELECT * FROM tbl_reserva where estado_reserva = 1 and id_mesa >= 17 and id_mesa <= 24 and fecha_inicio >= CURDATE() and  hora_reserva <> CURTIME() ORDER BY fecha_inicio ASC');
            $stmt->execute();
            ...
            $stmt = $pdo->prepare("SELECT * FROM tbl_reserva where estado_reserva = 1 and id_mesa >= 17 and id_mesa <= 24 and fecha_inicio >= CURDATE() and  hora_reserva <> CURTIME() and id_mesa like '%$table%' and fecha_inicio like '%$dia%' and hora_reserva like '%$hora%' ORDER BY fecha_inicio ASC");
            $stmt->execute();
            $sentencia=$stmt->fetchAll(PDO::FETCH_ASSOC);
